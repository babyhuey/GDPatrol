    ...
    mock_bedrock.invoke_model.return_value = {
        "body": MagicMock(
            read=lambda: json.dumps(
                {"content": [{"text": "This is a security analysis."}]}
            ).encode()
        )
    }
...
def test_lambda_handler_rds_finding(
    ...
    with patch("builtins.open", MagicMock()) as mock_open, patch(
        ...
        "GDPatrol.lambda_function.blacklist_ip"
    ) as mock_blacklist:
        mock_open.return_value.__enter__.return_value.read.return_value = (
            test_config_path.read_text()
        )
...
def test_whitelist_ip(mock_ec2_client):
    ...
    entries = mock_ec2_client.describe_network_acls(
        NetworkAclIds=[nacl_id]
    )["NetworkAcls"][0]["Entries"]
    assert not any(
        e.get("CidrBlock") == "192.168.1.1/32" and e["RuleAction"] == "deny"
        for e in entries
    )
...
def test_quarantine_instance(mock_ec2_client):
    ...
    vpc = mock_ec2_client.create_vpc(CidrBlock="10.0.0.0/16")
    subnet = mock_ec2_client.create_subnet(
